            ...
            <div class="wrapper">
              <template v-if="getAddedProduct">
                ...
                <p v-if="getProducts.length > 0" class="added-product ">
                 Рекомендуемый товар добавлен в корзину
                </p>
              </template>
              <ProductsList class="products" :products-from-cart="getProducts" />
            </div>
            <div>Всего: {{ getAmount | round }} руб</div>
            <div class="bottom">
              <a class="close" @click.prevent="$modal.hide('customer-cart')">
                Закрыть
              </a>
    ...
    getAmount() {
      ...
      if (this.getProductsInCart && this.getProductsInCart.length > 0) {
        this.getProductsInCart.forEach((product) => {
          amount += parseFloat(product.meta.price) * parseInt(product.amount)
        })
        return amount
      } else {
        return 0
